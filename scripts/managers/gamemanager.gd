...
func connect_skill_signals():
	...
	get_skill_by_name("Blood Count").connect("assassin_damage", Callable(self, "_on_assassin_damage"))
...
func set_available_actions() -> void:
	var option_button = $GUI.get_node("ActionSelection")
...
func _on_player_doubted(doubting_player: Player) -> void:
	...
	for card in current_player.cards:
		if card.skill1.sname == p_action_name or card.skill2.sname == p_action_name:
			doubting_player.coins = doubting_player.coins - 5
			await get_skill_by_name(current_player.requested_action).action(current_player)
...
func _on_assassin_damage(skill: Skill):
...
func next(log_text: String = "") -> void:
	...
	set_log(log_text)
	$GUI.get_node("ActionSelection").disabled = false
	$GUI.get_node("ActionSelection").selected = 0
